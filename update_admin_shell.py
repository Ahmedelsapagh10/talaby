...
with open('lib/features/admin/presentation/admin_shell.dart', 'r') as f:
    content = f.read()
...
class _AdminSidebar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final theme = Theme.of(context);
    // Use a dark sidebar for premium feel
    final sidebarColor = theme.brightness == Brightness.light ? const Color(0xFF2E2910) : theme.colorScheme.surface;
    final textColor = theme.brightness == Brightness.light ? Colors.white : theme.colorScheme.onSurface;
    final selectedTileColor = theme.brightness == Brightness.light ? Colors.white.withValues(alpha: 0.1) : theme.dividerColor;
      ...
      child: Column(
        ...
        children: [
          ...
          ...AdminSection.values.map(
            (item) => Padding(
              padding: const EdgeInsets.symmetric(horizontal: AppTokens.s16, vertical: 2),
              child: ListTile(
                leading: Icon(_iconFor(item), color: section == item ? textColor : textColor.withValues(alpha: 0.6)),
                title: Text(
                  _labelFor(item),
                  style: theme.textTheme.titleSmall?.copyWith(
                    color: section == item ? textColor : textColor.withValues(alpha: 0.6),
                    fontWeight: section == item ? FontWeight.bold : FontWeight.normal,
                  )
                ),
                selected: section == item,
                selectedTileColor: selectedTileColor,
                shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(AppTokens.r12)),
          ...
          Padding(
            ...
            child: ListTile(
              leading: Icon(PhosphorIconsRegular.signOut, color: textColor.withValues(alpha: 0.6)),
              title: Text('sign_out'.tr(), style: theme.textTheme.titleSmall?.copyWith(color: textColor.withValues(alpha: 0.6))),
              shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(AppTokens.r12)),
...
with open('lib/features/admin/presentation/admin_shell.dart', 'w') as f:
    f.write(replacement)
